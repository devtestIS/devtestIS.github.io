...
gulp.task('minify_scripts', function () {
    return gulp.src('bower_components/bootstrap-sass/assets/javascripts/bootstrap/*.js')
        ...
        .pipe(gulp.dest('bower_components/bootstrap-sass/assets/javascripts/bootstrap/'));
});


// Минификация стилей
gulp.task('minify_styles', function () {
    return gulp.src(['bower_components/jquery-ui/themes/base/*.css', '!**/*.min.css'])
        .pipe(cleanCss({compatibility: 'ie8'}))
        .pipe(rename({suffix: '.min'}))
        .pipe(gulp.dest('bower_components/jquery-ui/themes/base/'));
});
...
gulp.task('build_modernizr', function (cb) {
    ...
    var modernizr = require(path.join(process.cwd(), 'bower_components', 'modernizr', 'lib', 'cli')),
    ...
    modernizr.build(config, function (output) {
        fs.writeFile(path.join(process.cwd(), 'bower_components', 'modernizr', 'modernizr-custom.min.js'), output, cb);
    });
});
...
gulp.task('minify_html5shivrespond', function () {
    return gulp.src(['bower_components/html5shiv/dist/html5shiv.min.js',
        ...
        .pipe(gulp.dest('bower_components/html5shiv/'));
});
...
gulp.task('make_index', function (cb) {
    ...
    if (pages.length > 0) {
        ...
        html.push(pages.map(function (page) {
          ...
          return '<li><a href="' + page.replace(/\\/g, '/') + '">' + title + '</a></li>';
        }).join(''));
...
gulp.task('make_archive', function () {
    var now = (new Date()).toISOString().replace('T', '___').replace(/\./g, '___').replace(/:/g, '-'),
        name = path.basename(__dirname) + '-' + now + '.zip';
