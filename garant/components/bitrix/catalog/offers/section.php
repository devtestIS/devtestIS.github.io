...
$offerIblockId = Offer::getIblockId();
...
?><div class="sticky row"><?
	...
	?><div class="sticky__inner col-xs-12 col-sm-8 col-md-9"><?
		...
		if ($isFilter)
		{
			...
			$arFilterElements = array_merge($arFilterElements, $arrFilter);
			$dbResElements = CIBlockElement::GetList(array(), $arFilterElements, false, false, array('PROPERTY_CITY'));
			while($arElement = $dbResElements->GetNext())
			{
				if (!in_array($arUniqCities, $arElement['PROPERTY_CITY_VALUE']))
				{
					$arUniqCities[] = $arElement['PROPERTY_CITY_VALUE'];
				}
			}
			...
			$APPLICATION->IncludeComponent(
				...
				array(
					...
					"IBLOCK_ID" => INTERVOLGA_GARANT_CATALOG_COMPANIES == 'Y' ? $companyIblockId : $offerIblockId,
