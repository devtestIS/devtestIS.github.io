...
document.addEventListener('OnDocumentHtmlChanged', function () {
	if ($('#select-cities').length && typeof TagsArray !== 'undefined') {
		$('#select-cities').selectize({
			plugins: ['remove_button'],
			delimiter: ',',
		});

		var input = $('#select-cities-selectized'),
			selectize = $("#select-cities")[0].selectize;

		for (var i = 0; i < TagsArray.length; i++) {
			selectize.addItem(TagsArray[i]);
		}

		selectize.on('item_add', function (event) {
			var cityName = event;
			if (cityName) {
				$('[data-city-name="' + cityName + '"]').prev().click();
			}
		});
