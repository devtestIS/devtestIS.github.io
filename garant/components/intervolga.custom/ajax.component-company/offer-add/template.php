...
<? if ($arParams["SHOW_BUTTON"] == "Y"): ?>
	<button class="add-offer pull-right" type="button" href="javascript:void(0)" data-href="<?=$arResult["AJAX_VIEW_LINK"]?>" id="link<?=$arResult["ID"]?>" data-target="#<?=$arResult["ID"]?>"><?=$arParams["TITLE"]?></button>
<? endif ?>
<?ob_start();?>
...
<?$APPLICATION->AddViewContent('offer_add', ob_get_clean())?>
