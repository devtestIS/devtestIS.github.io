	...
	foreach($arCategory["ITEMS"] as $i => $arItem) {
		...
		if ($arItem["MODULE_ID"] == 'iblock') {
			if (stripos($arItem["ITEM_ID"], 'S') !== 0) {
				...
				if ($element && $element['DETAIL_PICTURE']) {
					$element['DETAIL_PICTURE'] = CFile::GetFileArray($element['DETAIL_PICTURE']);
					$element['DETAIL_PICTURE'] = ResizeImageGetIV(
						$element['DETAIL_PICTURE'], 
						array('height' => 80, 'width' => 80),
						BX_RESIZE_IMAGE_EXACT
					);
				}
				$arResult["CATEGORIES"][$category_id]["ITEMS"][$i]['ELEMENT'] = $element;

				$arResult['SEARCH_COUNTER']['ELEMENTS']++;
			}else{
				$arResult['SEARCH_COUNTER']['SECTIONS']++;
			}
		}
	}
}
...
if (($q = $_GET["q"]) && CSite::InDir($arResult["FORM_ACTION"]))
{
	$arResult["QUERY"] = htmlspecialchars($q);
}
?>