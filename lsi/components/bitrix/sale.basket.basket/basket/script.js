...
{
	var items = {};
	$('#basket_order_form > .list-orders__item.list-orders__item_product').each(function () {
		var $this = $(this);
		var itemId = $this.attr('data-item-id');
		items[itemId] = BX('QUANTITY_' + itemId).value;
	});
	this.currentQuantity = items;
};
...
function recalcBasketAjax(params)
{
	...
	var property_values = {},
		action_var = BX('action_var').value,
		items = $('#basket_order_form > .list-orders__item.list-orders__item_product'),
		postData,
		i;
	...
	BX.ajax({
		...
		onsuccess: function(result)
		{
			...
			if(result.BASKET_DATA.ERROR_MESSAGE){
				location.reload();
			}

			var sum = $('#basket_order_form_sum');
			sum.find('.list-orders__price').html(result.BASKET_DATA.allSum_FORMATED);
			if(result.BASKET_DATA.DISCOUNT_PRICE_ALL > 0) {
				sum.find('.list-orders__old-price').html(result.BASKET_DATA.PRICE_WITHOUT_DISCOUNT);
				sum.find('.list-orders__old-price').show();
			}else{
				sum.find('.list-orders__old-price').hide();
			}

			var delivery = sum.find('.list-orders__shipping');
			if(showDeliveryPrice){
				delivery.find('span').html(result.BASKET_DATA.allSum >= 10000 ? 'бесплатно' : '200 р');
				delivery.show();
			}else{
				delivery.hide();
			}
...
$(function () {
	$('#basket_order_form').on('click', '[data-href-remove]', function(){
		var $this = $(this);
		var product = $this.closest('.list-orders__item_product');

		$.post($this.attr('data-href-remove'), {}, function(){
			product.remove();
			recalcBasketAjax({});
