					...
					<div id="order_form_contact_info_collapse" class="collapse in">
						<div id="order_form_contact_info">
							...
							foreach ($arResult["ORDER_PROP"]["USER_PROPS_Y"] as $prop) {
								$val = $prop["VALUE"] ?: $prop["DEFAULT_VALUE"];
								if(!$val && !empty($prop["VARIANTS"])){
									foreach ($prop["VARIANTS"] as $var) {
										$val = $var['CODE'] ?: $var['ID'];
										break;
									}
								}
								?><input type="hidden" name="<?=$prop["FIELD_NAME"]?>" value="<?=$val?>" /><?
							}
							?>
						</div>
					...
					foreach($arResult['BASE_DELIVERY_LOCATION'] as $location){
						...
						<div class="radio radio_custom radio_inline">
							<label>
								...
								<span class="radio__input"></span><?=$location['NAME']?>
							</label>
						</div>
						<?
					}
