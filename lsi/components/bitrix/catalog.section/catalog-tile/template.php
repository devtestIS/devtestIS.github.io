...
if (!empty($arResult['ITEMS'])) {
	...
	<? if ($arParams['IS_SLIDER_CLASS']){ ?><div class="<?=$arParams['IS_SLIDER_CLASS']?>" data-speed="<?=$arParams["SLIDER_TIME"]?>"><? }?>
	<?
	...
	foreach ($arResult['ITEMS'] as $key => $arItem) {
		...
		<div <?= $archiv ? 'style="opacity: 0.5"' : ''?> class="<? if ($arParams['IS_SLIDER_CLASS']){echo $arParams['IS_SLIDER_CLASS']."__item";} ?> <?=$arParams['FILT_ELEMENT_CSS_CLASS_IN_ROW'] ?: 'col-lg-3 col-sm-6'?>">
			<div class="card-product <?=$arParams['FILT_ELEMENT_CSS_CLASS'] ?: ''?>" id="<?=$strMainID?>">
				...
				<div class="card-product__label-line">
					...
					if(in_array('AVAILABILITY', $arParams['VIEW_LABEL'])) {
						?>
						<div
							class="card-product__label card-product__label_type_<?=$arItem['LABELS']['AVAILABILITY']['VIEW']
								? 'availability' : 'not-available'?>">
							<?=$arItem['LABELS']['AVAILABILITY']['VIEW'] ? 'В наличии' : 'Под заказ'?>
						</div>
						<?
					}
					?>
                ...
                <div class="star">
	                ...
	                <?if($arParams["SHOW_FAVORITE"] == "Y" && !$archiv):?>
				...
				<div class="card-product__bottom">
					<?if(!$archiv):?>
						...
						<div
							class="card-product__price<? if ($withOldPrice) { ?> card-product__price_new<? } ?><? if (mb_strlen(
									'' . $arItem['MIN_PRICE']['DISCOUNT_VALUE']
								) > TEMPLATE_MAX_COUNT_IN_NORMAL_NUMBER
	...
	if ($arParams['IS_SLIDER_CLASS']){ ?></div><? }
