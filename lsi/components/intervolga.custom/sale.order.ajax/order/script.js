...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        showDeliveryTime: function(result) {
            var Items = result.order.GRID.ROWS;
            var radio = $('[name="DELIVERY_ADDRESS_1"]:checked');
            var currentStore = radio.val();
            var deliveryId = $('[name="DELIVERY_ID"]:checked');
            var self = this;
            var zones = $('[data-store-inf]');
            $('[name="DELIVERY_ID"]').on('change', function () {
               if(jQuery.inArray(+$(this).val(), self.result['SELF_DELIVERY']) < 0) {
                   zones.hide();
               } else {
                   zones.show();
               }
            });
            if(jQuery.inArray(+deliveryId.val(), result.order['SELF_DELIVERY']) < 0) {
                zones.hide();
            } else {
                zones.show();
            }
            for(var id in Items) {
                if(Items[id]['STORES']) {
                    var html = '';
                    for(var i in Items[id]['STORES']) {
                        var store = Items[id]['STORES'][i];
                        ...
                        if(store['COUNT'] > 0) {
                            count = 'Сегодня'
                        } else if(Items[id]['COUNT_IN_STORES'] > 0) {
                            count = 'через '+store['PRINT_DELIVERY_DAYS'];
                        } else {
                            count = 'через '+store['PRINT_TOTAL_DELIVERY_DAYS'];
                        }
                        html += '<div class="list-orders__id text-muted">' + store['NAME'] + ': Самовывоз ' + count + '</div>';
                    }
                    if(html) {
                        $('[data-item-id="'+id+'"]').find('[data-store-inf]').html(html);
                    }
                }
            }

            $('[name="DELIVERY_ADDRESS_1"]').on('change', function () {
                var Items = self.result.GRID.ROWS;
                ...
                for(var id in Items) {
                    if(Items[id]['STORES']) {
                        ...
                        for(var i in Items[id]['STORES']) {
                            ...
                            if(store['COUNT'] > 0) {
                                count = 'Сегодня'
                            } else if(Items[id]['COUNT_IN_STORES'] > 0 && result.order.IS_DEFAULT_LOCATION) {
                                count = "через "+store['PRINT_DELIVERY_DAYS'];
                            } else {
                                count = "через "+store['PRINT_TOTAL_DELIVERY_DAYS'];
                            }
...
function updateOrderForm(data) {
    ...
    var orderWarningZone = $('#order-warning-zone');
    if (data.message) {
        orderWarningZone.text(data.message);
        orderWarningZone.show();
    } else {
        orderWarningZone.hide();
    }

    var baseDeliveryZone = $('#select-city-order');
    baseDeliveryZone.find('.pvx').hide();
    var baseDeliveries = data.order.BASE_DELIVERY_LOCATION;
    for (var k in baseDeliveries) {
        var item = baseDeliveryZone.find('[value="' + baseDeliveries[k]['CODE'] + '"]');
        if (item.length > 0) {
            item.closest('.pvx').show();
        }
    }
    ...
    for (var key in data.order.ORDER_PROP.properties) {
        ...
        if (data.order.ORDER_PROP.properties[key].CODE == 'DELIVERY_ADDRESS') {
            html += '<input type="hidden"' +
                ...
                ' />';
            $('[name="CITY_ORDER"][value="' + v + '"]').each(function () {
                var $this = $(this);
                $('[name="' + $this.attr('name') + '"]').prop('checked', false);
                $this.prop('checked', true);

                var radio = $('[name="DELIVERY_ADDRESS_0"]');
                radio.val($this.val());
                $('#deliv_name').html($this.attr('data-name'));
    ...
    for (var key in data.order.DELIVERY) {
        ...
        if (data.order.DELIVERY[key].CHECKED == 'Y') {
            ...
            if (data.order.DELIVERY[key].ID == 2 || data.order.DELIVERY[key].ID == 6) { //самовывоз
                var pickup = '';
                var first = true;
                if (typeof(delivery_map) == 'object' && delivery_map !== null) {
                ...
                for (var key in data.order.STORES_FOR_DELIVERY) {
                    pickup += '<div class="radio radio_custom">' +
                        ...
                        ' value="' + data.order.STORES_FOR_DELIVERY[key]['ID'] + '"' +
                        ' data-change-local="Y"' +
...
$(document).ready(function () {
   ...
   var modal = $('#select-city-order');
    modal.find('[data-modal-ok]').on('click', function () {
        var input = $('[name="CITY_ORDER"]:checked');
        var radio = $('[name="DELIVERY_ADDRESS_0"]');
        radio.val(input.val());
        $('#deliv_name').html(input.attr('data-name'));
        modal.modal('hide');
        radio.click();
        radio.change();
    })
